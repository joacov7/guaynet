...
export default function Invoices() {
  ...
  const columns = [
    ...
    { title: "Monto", dataIndex: "amount", key: "amount", render: (v: number) => formatARS(v) },
    { title: "Vencimiento", dataIndex: "due_date", key: "due_date", render: (v: string) => dayjs(v).format("DD/MM/YYYY") },
    {
      title: "Estado",
      dataIndex: "status",
      key: "status",
      render: (s: InvoiceStatus) => <Tag color={statusConfig[s].color}>{statusConfig[s].label}</Tag>,
    },
    {
      title: "",
      key: "actions",
    ...
    <div>
      ...
      >
        <Form
          form={payForm}
          layout="vertical"
          onFinish={(v) => addPaymentMutation.mutate({
            id: payModal!.id,
        ...
        >
          ...
          <Form.Item name="reference" label="Referencia / N° transferencia"><Input /></Form.Item>
